...
Download from the [Tatoeba corpus](https://opus.nlpl.eu/Tatoeba.php) 
and place at `data/fra.txt` before running.
...
The fine-tuned model matches the base Helsinki-NLP/opus-mt-en-fr 
model on BLEU while marginally outperforming it on chrF — the more 
morphologically sensitive metric. This suggests fine-tuning on the 
Tatoeba subset preserved translation quality while adapting the 
model's outputs toward the style and register of the corpus.

The increased latency in the fine-tuned model (~57% slower) is a 
known trade-off of fine-tuning and would be a consideration in any 
production deployment.

### Example Translations

| Source | Reference | Fine-tuned | Base model |
|--------|-----------|------------|------------|
| Just promise me you're going to be careful. | Promettez-moi juste que vous ferez attention. | Promettez-moi simplement que vous allez être prudentes. | Promets-moi d'être prudent. |
| I wrote this song just for you. | J'ai écrit cette chanson, juste pour toi. | J'ai écrit cette chanson juste pour toi. | J'ai écrit cette chanson juste pour toi. |
| Tom can't help. | Tom n'y peut rien. | Tom ne peut pas aider. | Tom ne peut rien faire. |

### Notes on Evaluation

- BLEU measures n-gram overlap with reference translations (0–100, 
  higher is better). Scores above 50 are considered strong for 
  constrained domain translation.
- chrF measures character-level similarity, making it more sensitive 
  to morphological variation — particularly relevant for French, 
  where verb conjugation and agreement produce significant surface 
  form variation.
- The fine-tuned model was trained on 50,000 sentence pairs for 3 
  epochs with a learning rate of 3e-5.